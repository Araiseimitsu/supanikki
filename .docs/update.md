...
- **シート切り替え機能を追加**:
  - ホットキー押下状態がスタックする場合に再登録でリセット
  - ホットキー登録失敗時は入力用ショートカットのみでフォールバック
  - 現在のシート名を小さく表示（入力UI右側）
  - 同一スプレッドシート内のシートを選択可能（トレイメニュー/設定）
  - 次/前シートへ切り替えるホットキーを追加（`sheet_next_hotkey` / `sheet_prev_hotkey`）
  - セットアップ/使用方法の説明を更新（dist README）
